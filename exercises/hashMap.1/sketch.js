...
function setup() {
    ...
    loadStrings('poem.txt', callback); // data is passed into callback as argument
...
function callback(poem) {
    ...
    for(var i in poem) { 
        var li = poem[i].split(' ');
        for (var k in li) {
            var clean = li[k].replace(/[.,-?!@#$%^&*()_~{}]/g, ''); // forward slash signifies look for these patters, /g signifies globally
